...
class News extends Commer
{

	public function index()
	{
    	$site= Db::query("select * from think_column where id=5");//查询当前栏目的seo
		$webset =$this->webset;

		$page =$webset['inpage'];
			$news = Db::name('news')->where('catid',5)->where('status',0)->order('id desc')->paginate($page)->each(function($item,$key){
	...
	public function article()
	{
		...
		$data = Db::query("select * from think_news where id='$id' and catid=5");

		$prenew = Db::query("select * from think_news where id <'$id' and catid=5 order by id desc limit 1");//上一篇

		$nexnew = Db::query("select * from think_news where id > '$id' and catid=5 order by id Asc limit 1");//下一篇
